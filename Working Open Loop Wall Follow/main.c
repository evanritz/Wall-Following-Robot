...
{    
     ...
     */   
    //WALL FOLLOWING ALGORITHM OPEN LOOP
...
//    }
        ...
        if (PID_CTRL_R.isStopped) {
            PWM_START(&PID_CTRL_R);
        }
        if (PID_CTRL_L.isStopped) {
            PWM_START(&PID_CTRL_L);
        }
        ...
        // turn left
            ...
            PID_CTRL_L.target_speed = PID_CTRL_R.target_speed * .2;
            strcpy(&algo, "MOVING LEFT");
        }
    }
    else {
         // turn right
        PID_CTRL_R.target_speed = PID_CTRL_L.target_speed * .2;
        strcpy(&algo, "MOVING RIGHT");
