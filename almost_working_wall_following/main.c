...
{    
    ...
    if (PID_CTRL_1.IC_update) {
        PID_UPDATE(&PID_CTRL_1);
        PWM_UPDATE(&PID_CTRL_1);
    }
    
    if (PID_CTRL_2.IC_update) {
        PID_UPDATE(&PID_CTRL_2);
        PWM_UPDATE(&PID_CTRL_2);
    }
        
    
    float FRONT_CENTER = OPT0_DIST();
    float FRONT_RIGHT = OPT2_DIST();
    float BACK_RIGHT = OPT1_DIST();
    
    if (FRONT_CENTER < MIN_WALL_DIST) {
        PWM_STOP(&PID_CTRL_1);
        PWM_STOP(&PID_CTRL_2);
//        PID_CTRL_1.target_speed = 0;
//        PID_CTRL_2.target_speed = 0;
        strcpy(&algo, "WALL NEAR Front");
    }
    else if (FRONT_RIGHT > MAX_WALL_DIST && BACK_RIGHT > MAX_WALL_DIST) {
        PWM_STOP(&PID_CTRL_1);
        PWM_STOP(&PID_CTRL_2);
//        PID_CTRL_1.target_speed = 0;
//        PID_CTRL_2.target_speed = 0;
        strcpy(&algo, "IDK WYD LOL");
    }
    else if (FRONT_RIGHT <= TARGET_DIST + UPPER_TOL) {
        float DIFF = FRONT_RIGHT - BACK_RIGHT;
        
        if (PID_CTRL_1.isStopped) {
            PWM_START(&PID_CTRL_1);
        }
        if (PID_CTRL_2.isStopped) {
            PWM_START(&PID_CTRL_2);
        }
        
        // go straight
        if (DIFF <= UPPER_TOL && DIFF >= LOWER_TOL) {
            PID_CTRL_1.target_speed = 1.425;
            PID_CTRL_2.target_speed = 1.5;
            strcpy(&algo, "MOVING FORWARD");
        }
        // turn right
        else if (DIFF > UPPER_TOL) {
            PID_CTRL_1.target_speed = PID_CTRL_2.target_speed * .6;
            strcpy(&algo, "MOVING RIGHT");
        }
        // turn left
        else if (DIFF < LOWER_TOL) {
            PID_CTRL_2.target_speed = PID_CTRL_1.target_speed * .6;
            strcpy(&algo, "MOVING LEFT");
        }
    }
    else {
        
        strcpy(&algo, "ELSE IDK");
        if (PID_CTRL_1.isStopped) {
            PWM_START(&PID_CTRL_1);
        }
////         
////        PWM_STOP(&PID_CTRL_1);
        PID_CTRL_1.target_speed = PID_CTRL_2.target_speed * .2;
...
void __ISR(_INPUT_CAPTURE_2_VECTOR, ipl5) _IC2_IntHandler(void) 
{
    ...
    IC_UPDATE(&PID_CTRL_1, buffer_data);
...
void __ISR(_INPUT_CAPTURE_3_VECTOR, ipl5) _IC3_IntHandler(void) 
{
    ...
    IC_UPDATE(&PID_CTRL_2, buffer_data);
...
//	DelayMs(2000);
	...
	{	
        ...
        sprintf(strout, "PID=%.2f", PID_CTRL_1.pid_val);  
        //char sym = '>';
        //if (PID_CTRL_1.IC_curr < PID_CTRL_1.IC_prev)
        //    sym = '<';
        sprintf(strout_2, "CS=%.2f TS=%.2f", PID_CTRL_1.curr_speed, PID_CTRL_1.target_speed);
//        sprintf(strout, "Front=%.2f", OPT0_DIST());      
//        sprintf(strout_2, "FR=%.2f BR=%.2f", OPT2_DIST(), OPT1_DIST());
...
void AppInit() {
    ...
    PID_CTRL_1 = PID_INIT(&OC2R, &OC2RS, 3, 0.5, 5, 10000, -10000, 1.5);
    PID_CTRL_2 = PID_INIT(&OC3R, &OC3RS, 3, 0.5, 5, 10000, -10000, 1.5);
